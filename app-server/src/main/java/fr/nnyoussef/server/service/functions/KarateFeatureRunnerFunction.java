...
import static fr.nnyoussef.server.service.enums.TestResultsEvent.*;
...
public final class KarateFeatureRunnerFunction extends BaseFunction implements Function<FeatureRunnerRequestBody, Flux<ServerSentEvent<String>>> {
    ...
    private static final String TEST_END_EVENT_MESSAGE_FORMAT = "%d Passed | %d Failed. Executed in %s seconds";
    ...
    public Flux<ServerSentEvent<String>> apply(FeatureRunnerRequestBody featureRunnerRequestBody) {
        return Flux.create(sink -> {
            ...
            Runner.Builder<?> builder = Runner.builder()
                    ...
                    .suiteReports(null)
                    .debugMode(false);
            featureRunnerRequestBody.testParams().forEach(builder::systemProperty);
            builder.parallel(1);
        });
    }
    ...
    private final class TestRunnerEventListener implements RuntimeHook {
        ...
        public void afterFeature(FeatureRuntime fr) {
            Double d = fr.result.getDurationMillis();

            String data = format(TEST_END_EVENT_MESSAGE_FORMAT, fr.result.getPassedCount(), fr.result.getFailedCount(), d);
            ServerSentEvent<String> stepEvent = ServerSentEvent.<String>builder()
                    .event(TEST_END.name())
                    .data(data)
                    .build();
